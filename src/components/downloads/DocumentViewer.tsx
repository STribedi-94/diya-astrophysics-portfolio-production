 ...
 *
 * The heavy document itself is mounted only while the dialog is open.
 * The viewer preserves the page scroll position when closing and uses
 * the full available dialog height for the document preview.
 */
  ...
  return (
    ...
    >
      <DialogContent
        ...
        className={cn(
          "flex min-h-0 flex-col gap-0 overflow-hidden border-white/10 bg-[oklch(0.12_0.04_265_/_0.96)] p-0 backdrop-blur-xl",
          expanded
            ? "h-[100dvh] w-screen max-w-none rounded-none sm:max-w-none"
            : "h-[92dvh] w-[96vw] max-w-5xl rounded-2xl",
        )}
      >
        <DialogHeader className="shrink-0 space-y-1 border-b border-white/10 px-4 py-3 text-left sm:px-6">
          ...
          <DialogDescription className="sr-only">
            Document viewer for {record.title}. Press Escape to
            close.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="shrink-0 flex flex-wrap items-center gap-2 border-t border-white/10 px-4 py-3 sm:px-6">
          {record.fileUrl && (
            <a
              href={record.fileUrl}
              download={record.downloadName}
